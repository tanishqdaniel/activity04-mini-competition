...
training_data <- training_data %>% 
  mutate(
    Facebook = ifelse(Facebook == -1 | Facebook == 0 ,NA,Facebook),
    GooglePlus = ifelse(GooglePlus == -1 | GooglePlus == 0 ,NA,GooglePlus),
    Linkedin = ifelse(LinkedIn == -1 | LinkedIn == 0 ,NA,LinkedIn))
```
...
plot(training_data$Facebook_new, training_data$SentimentHeadline, main = "Scatter plot of Facebook vs SentimentHeadline Variable", xlab = "Facebook", ylab = "SentimentHeadline")

plot(training_data$GooglePlus_new, training_data$SentimentHeadline, main = "Scatter plot of GooglePlus vs SentimentHeadline Variable", xlab = "GooglePlus", ylab = "SentimentHeadline")

plot(training_data$Linkedin_new, training_data$SentimentHeadline, main = "Scatter plot of LinkedIn vs SentimentHeadline Variable", xlab = "LinkedIn", ylab = "SentimentHeadline")
...
training_data <- training_data %>%
  mutate(log_SentimentTitle =  log(SentimentTitle))
...
training_data <- training_data %>%
  mutate(GooglePlus =  log(GooglePlus))
...
adjrsquared_model4<- summary(model4)$adj.r.squared
cat("Mean Squared Error:", mse_model4, "\n")
cat("Root Mean Squared Error:", rmse_model4, "\n")
cat("R-squared:", rsquared_model4, "\n") 
cat("Adjusted R-squared:", adjrsquared_model4, "\n")
```
```{r model5}
model5 <- lm(SentimentHeadline ~ SentimentTitle + LinkedIn + Facebook, data = training_data)
```

```{r model4 test}
predictions <- predict(model5, newdata = testing_data)

mse_model5 <- mean((testing_data$SentimentHeadline - predictions)^2)
rmse_model5 <- sqrt(mse_model5)
rsquared_model5<- summary(model5)$r.squared
adjrsquared_model5<- summary(model5)$adj.r.squared
cat("Mean Squared Error:", mse_model5, "\n")
cat("Root Mean Squared Error:", rmse_model5, "\n")
cat("R-squared:", rsquared_model5, "\n") 
cat("Adjusted R-squared:", adjrsquared_model5, "\n")
```
